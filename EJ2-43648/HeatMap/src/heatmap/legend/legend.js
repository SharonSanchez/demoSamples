...
define(["require", "exports", "@syncfusion/ej2-base", "../utils/helper", "../utils/helper", "../model/base", "../utils/helper", "../model/theme", "@syncfusion/ej2-svg-base"], function (require, exports, ej2_base_1, helper_1, helper_2, base_1, helper_3, theme_1, ej2_svg_base_1) {
    "use strict";
    ...
    var LegendSettings = (function (_super) {
        ...
        __decorate([
            ej2_base_1.Complex({ text: '', textStyle: theme_1.Theme.titleFont }, base_1.Title)
        ], LegendSettings.prototype, "title", void 0);
        __decorate([
            ej2_base_1.Property('Right')
    ...
    var Legend = (function () {
        function Legend(heatMap) {
            this.maxLegendLabelSize = new helper_2.Size(0, 0);
            ...
            this.legendSize = 10;
            this.previousOptions = new helper_1.GradientPointer(0, 0, 0, 0, 0, 0);
            this.listPerPage = 0;
            this.numberOfPages = 1;
            this.listWidth = 0;
            this.fillRect = new helper_3.Rect(0, 0, 0, 0);
            this.legendRect = new helper_3.Rect(0, 0, 0, 0);
            this.currentPage = 1;
            this.lastList = [];
            this.navigationCollections = [];
            this.pagingRect = new helper_3.Rect(0, 0, 0, 0);
            ...
            this.drawSvgCanvas = new helper_1.DrawSvgCanvas(heatMap);
        }
        ;
        ...
        Legend.prototype.renderLegendItems = function () {
            ...
            var rectItems = new helper_3.RectOption(heatMap.element.id + '_LegendBound', 'none', tempBorder, 1, this.legendGroup);
            ...
            var rectItemsSvg = new helper_3.Rect(legendBound.x, legendBound.y, legendBound.width, legendBound.height);
            var fill;
            if (heatMap.paletteSettings.type === 'Fixed') {
                ...
                for (var i = 0; i < colorCollection.length; i++) {
                    var visibility = !ej2_base_1.isNullOrUndefined(this.visibilityCollections[i]) ? this.visibilityCollections[i] : true;
                    heatMap.toggleValue.push(new helper_2.ToggleVisibility(visibility, colorCollection[i].value, colorCollection[i].startValue, colorCollection[i].endValue));
                }
            }
            if (heatMap.paletteSettings.type === 'Gradient' || (heatMap.paletteSettings.type === 'Fixed' &&
                ...
                if (heatMap.paletteSettings.type === 'Gradient') {
                    if (heatMap.enableCanvasRendering) {
                        ...
                        for (var i = 0; i < heatMap.legendColorCollection.length; i++) {
                            if (this.heatMap.isColorRange && this.heatMap.paletteSettings.type === 'Gradient') {
                                ...
                                gradientColor = new helper_3.GradientColor(heatMap.legendColorCollection[i].color, gradientPercentage + '%');
                        ...
                        if (heatMap.horizontalGradient) {
                            gradientOptions = new helper_3.Gradient(heatMap.element.id + '_lineargradient', '0%', '100%', '0%', '0%');
                        }
                        else {
                            gradientOptions = new helper_3.Gradient(heatMap.element.id + '_lineargradient', '0%', '0%', '0%', '100%');
                        }
                    ...
                    var rectItem = new helper_3.RectOption(heatMap.element.id + '_Gradient_Legend', fill, tempBorder, 1, rectItemsSvg);
                ...
                var listRect = new helper_3.RectOption(heatMap.element.id + '_Gradient_Scale', 'none', tempBorder, 1, this.legendRectScale);
        ...
        Legend.prototype.calculateColorRange = function (i, cgradientColors) {
            ...
            if (this.heatMap.legendColorCollection[0].startValue > this.heatMap.dataSourceMinValue && i === 0) {
                gradientPercentage = (this.heatMap.dataSourceMinValue - this.legendMinValue) /
                    (this.legendMaxValue - this.legendMinValue) * 100;
                gradientPercentage = isNaN(gradientPercentage) ? 0 : gradientPercentage;
                gradientColor = new helper_3.GradientColor(heatMap.paletteSettings.fillColor.minColor, gradientPercentage + '%');
                cgradientColors.push(gradientColor);
                gradientPercentage = (heatMap.legendColorCollection[0].startValue - this.legendMinValue) /
                    (this.legendMaxValue - this.legendMinValue) * 100;
                gradientColor = new helper_3.GradientColor(heatMap.paletteSettings.fillColor.maxColor, gradientPercentage + '%');
                cgradientColors.push(gradientColor);
            }
            gradientPercentage = ((heatMap.legendColorCollection[i].startValue - this.legendMinValue) /
                (this.legendMaxValue - this.legendMinValue)) * 100;
            gradientPercentage = isNaN(gradientPercentage) ? 0 : gradientPercentage;
            gradientColor = new helper_3.GradientColor(heatMap.legendColorCollection[i].minColor, gradientPercentage + '%');
            cgradientColors.push(gradientColor);
            gradientPercentage = (heatMap.legendColorCollection[i].endValue - this.legendMinValue) /
                (this.legendMaxValue - this.legendMinValue) * 100;
            gradientColor1 = new helper_3.GradientColor(heatMap.legendColorCollection[i].maxColor, gradientPercentage + '%');
            cgradientColors.push(gradientColor1);
            if (this.heatMap.legendColorCollection[i].endValue !== ((i === this.heatMap.legendColorCollection.length - 1) ?
                this.heatMap.dataSourceMaxValue : this.heatMap.legendColorCollection[i + 1].startValue)) {
                gradientPercentage = (heatMap.legendColorCollection[i].endValue - this.legendMinValue) /
                    (this.legendMaxValue - this.legendMinValue) * 100;
                gradientColor2 = new helper_3.GradientColor(this.heatMap.paletteSettings.fillColor.minColor, (gradientPercentage) + '%');
                ...
                gradientPercentage = ((i === (this.heatMap.legendColorCollection.length - 1) ?
                    ...
                    (this.legendMaxValue - this.legendMinValue) * 100;
                gradientColor3 = new helper_3.GradientColor(this.heatMap.paletteSettings.fillColor.maxColor, (gradientPercentage) + '%');
        ...
        Legend.prototype.renderTitle = function (rect) {
            ...
            if (heatMap.legendSettings.title.text) {
                ...
                var titleSize = helper_2.measureText(title.text, title.textStyle);
                ...
                if (heatMap.legendSettings.title.textStyle.textOverflow === 'Trim') {
                    ...
                    text = helper_1.textTrim(maxWidth, text, title.textStyle);
                }
                if (!heatMap.horizontalGradient) {
                    padding = -(padding + titleSize.height / 4);
                    if (text.length !== 0 && heatMap.enableCanvasRendering) {
                        this.legendTitleTooltip.push(new helper_2.CanvasTooltip(title.text, new helper_3.Rect(rect.x, rect.y - titleSize.height, maxWidth, titleSize.height)));
                    }
                    options = new helper_1.TextOption(heatMap.element.id + '_legendTitle', new helper_1.TextBasic(rect.x, rect.y + padding, anchor, text, 0, 'translate(0,0)', dominantBaseline), title.textStyle, title.textStyle.color || heatMap.themeStyle.heatMapTitle);
                }
                else {
                    ...
                    if (text.length !== 0 && heatMap.enableCanvasRendering) {
                        this.legendTitleTooltip.push(new helper_2.CanvasTooltip(title.text, new helper_3.Rect(rect.x, yValue, maxWidth, titleSize.height)));
                    }
                    titleSize.width = rect.width < titleSize.width ? rect.width : titleSize.width;
                    options = new helper_1.TextOption(heatMap.element.id + '_legendTitle', new helper_1.TextBasic(rect.x + (rect.width / 2) - (titleSize.width / 2), y + padding, anchor, text, 0, 'translate(0,0)', dominantBaseline), title.textStyle, title.textStyle.color || heatMap.themeStyle.heatMapTitle);
                }
        ...
        Legend.prototype.renderSmartLegend = function () {
            ...
            this.legendRange = [];
            for (var i = 0; i < heatMap.legendColorCollection.length; i++) {
                var rectPosition = new helper_3.CurrentLegendRect(0, 0, 0, 0, '', '');
                if (heatMap.horizontalGradient) {
                ...
                smartLegendRect = new helper_3.Rect(legendX, legendY, legendWidth, legendHeight);
                var legendRange = new helper_2.LegendRange(0, 0, 0, 0, 0, true, 0);
                ...
                legendRange.visible = !ej2_base_1.isNullOrUndefined(this.visibilityCollections[i]) ? this.visibilityCollections[i] : true;
                ...
                if (!heatMap.legendColorCollection[i].isHidden) {
                    ...
                    var rectItem = new helper_3.RectOption(heatMap.element.id + '_Smart_Legend' + i, color, tempBorder, 1, smartLegendRect);
                    this.drawSvgCanvas.drawRectangle(rectItem, this.legend);
                    ...
                    var text = helper_2.getTitle(this.labelCollections[i], heatMap.legendSettings.textStyle, this.textWrapCollections[i]);
                    if (text.length !== 0 && heatMap.enableCanvasRendering) {
                        var elementSize = helper_2.measureText(this.labelCollections[i], heatMap.legendSettings.textStyle);
                        this.legendLabelTooltip.push(new helper_2.CanvasTooltip(this.labelCollections[i], new helper_3.Rect(rectPosition.x, rectPosition.y, elementSize.width, elementSize.height)));
                    }
                }
            }
        };
        ...
        Legend.prototype.renderLegendLabel = function (rect) {
            ...
            if (heatMap.legendSettings.showLabel && (heatMap.paletteSettings.type === 'Gradient' ||
                ...
                for (var i = 0; i < colorCollection.length; i++) {
                    ...
                    text = helper_2.getTitle(this.labelCollections[i], heatMap.legendSettings.textStyle, textWrapWidth);
                    elementSize = helper_2.measureText(text[0], heatMap.legendSettings.textStyle);
                    if (heatMap.paletteSettings.type === 'Fixed') {
                        ...
                        var textPosition = new helper_2.LegendRange(rectX, rectY, elementSize.width, elementSize.height, colorCollection[i].value, true, this.currentPage);
                        textPosition.visible = !ej2_base_1.isNullOrUndefined(this.visibilityCollections[i]) ? this.visibilityCollections[i] : true;
                    ...
                    if (this.labelCollections[i] !== '') {
                        if (text.length !== 0 && text[0].indexOf('...') !== -1 && heatMap.enableCanvasRendering) {
                            this.legendLabelTooltip.push(new helper_2.CanvasTooltip(this.labelCollections[i], new helper_3.Rect(labelX, labelY, elementSize.width, elementSize.height)));
                        }
                        var textBasic = new helper_1.TextBasic(labelX, labelY, anchor, text, 0, 'translate(0,0)', dominantBaseline);
                        var options = new helper_1.TextOption(heatMap.element.id + '_Legend_Label' + i, textBasic, heatMap.legendSettings.textStyle, heatMap.legendSettings.textStyle.color || heatMap.themeStyle.legendLabel);
        ...
        Legend.prototype.renderGradientPointer = function (e, pageX, pageY) {
            ...
            if (cellValue.toString() !== '') {
                ...
                options = new helper_1.PathOption(heatMap.element.id + '_Gradient_Pointer', 'gray', 0.01, '#A0A0A0', 1, '0,0', direction);
                if (!heatMap.enableCanvasRendering) {
        ...
        Legend.prototype.calculateLegendBounds = function (rect) {
            ...
            if (heatMap.paletteSettings.type === 'Fixed') {
                for (var i = 0; i < colorCollection.length; i++) {
                    var label = colorCollection[i].label ? colorCollection[i].label : this.heatMap.isColorRange ?
                        colorCollection[i].startValue.toString() + '-' + colorCollection[i].endValue.toString() : helper_2.formatValue(isCustom, format, colorCollection[i].value, this.format).toString();
                ...
                for (var i = 0; i < colorCollection.length; i++) {
                    var label = colorCollection[i].isHidden ? '' : colorCollection[i].label ? colorCollection[i].label :
                        this.heatMap.isColorRange ? colorCollection[i].startValue.toString() + '-' + colorCollection[i].endValue.toString() :
                            helper_2.formatValue(isCustom, format, colorCollection[i].value, this.format).toString();
                    var legendEventArg = { cancel: false, text: label, name: 'legendRender', };
            ...
            this.height = helper_3.stringToNumber(this.legendHeight, rect.height);
            this.width = helper_3.stringToNumber(this.legendWidth, rect.width);
            if (heatMap.horizontalGradient) {
        ...
        Legend.prototype.calculateListLegendBounds = function (rect) {
            ...
            if (heatMap.horizontalGradient) {
                for (var i = 0; i < heatMap.colorCollection.length; i++) {
                    ...
                    if (heatMap.legendSettings.showLabel) {
                        ...
                        size = helper_2.measureText(text, heatMap.legendSettings.textStyle).width;
                    }
                ...
                if (this.legendHeight === '') {
                    this.numberOfRows = Math.ceil(this.listWidth / helper_3.stringToNumber(this.legendWidth, rect.width));
                    this.numberOfRows = this.numberOfRows > 3 ? 3 : this.numberOfRows;
                    this.legendHeight = (this.listWidth > rect.width || this.listWidth > helper_3.stringToNumber(this.legendWidth, rect.width)) &&
            ...
            if (helper_3.stringToNumber(this.legendHeight, rect.height) < 50) {
                this.legendHeight = height;
            }
            if (helper_3.stringToNumber(this.legendWidth, rect.width) < 70) {
                this.legendWidth = '70';
            }
        };
        Legend.prototype.getMaxLabelSize = function () {
            ...
            if (!heatMap.legendSettings.showLabel || (heatMap.horizontalGradient && heatMap.paletteSettings.type === 'Fixed' &&
                ...
                for (var i = 0; i < heatMap.legendColorCollection.length; i++) {
                    var size = helper_2.measureText(this.labelCollections[i], heatMap.legendSettings.textStyle);
        ...
        Legend.prototype.calculateLegendSize = function (rect, legendTop) {
            ...
            var width;
            var height = helper_3.stringToNumber(this.legendHeight, rect.height);
            if (!heatMap.legendSettings.title.text) {
                width = helper_3.stringToNumber(this.legendWidth, rect.width);
            }
            else {
                width = this.width;
            }
            ...
            if (heatMap.horizontalGradient) {
                ...
                if (heatMap.xAxis.title.text !== '') {
                    axisTitlePadding = helper_2.measureText(heatMap.xAxis.title.text, heatMap.xAxis.textStyle).height + padding;
                }
                var axisHeight = axis[0].opposedPosition ? 0 : helper_2.sum(axis[0].xAxisMultiLabelHeight) + axis[0].maxLabelSize.height +
                ...
                if (heatMap.yAxis.title.text !== '') {
                    axisTitlePadding = helper_2.measureText(heatMap.yAxis.title.text, heatMap.yAxis.textStyle).height + padding;
                }
                var axisWidth = axis[1].opposedPosition ? helper_2.sum(axis[1].yAxisMultiLabelHeight) +
            ...
            this.legendGroup = new helper_3.Rect(left, top, width, height);
        ...
        Legend.prototype.measureListLegendBound = function (rect) {
            ...
            if (heatMap.horizontalGradient) {
                ...
                if (this.listHeight > rect.height || this.listHeight > this.height) {
                    var maxHeight = helper_3.stringToNumber(this.legendHeight, rect.height);
        ...
        Legend.prototype.renderPagingElements = function () {
            ...
            if (this.numberOfPages > 1) {
                ...
                var textSize = helper_2.measureText(text, heatMap.legendSettings.textStyle);
                var textX = rightArrowX - textSize.width - 15;
                var textBasic = new helper_1.TextBasic(textX, rightArrowY, 'start', text, 0, 'translate(0,0)', 'middle');
                var options = new helper_1.TextOption(heatMap.element.id + '_paging', textBasic, heatMap.legendSettings.textStyle, heatMap.legendSettings.textStyle.color || heatMap.themeStyle.legendLabel);
                this.drawSvgCanvas.createText(options, this.paginggroup, text);
                if (ej2_base_1.Browser.isIE && !heatMap.enableCanvasRendering) {
                    this.paginggroup.lastChild.setAttribute('dy', '0.6ex');
                }
                this.pagingRect = new helper_3.Rect(textX, rightArrowY - textSize.height / 2, textSize.width, textSize.height);
                var pagingTextRect = new helper_3.RectOption(heatMap.element.id + '_pagingText', 'none', { color: 'transparent', width: 0 }, 1, this.pagingRect);
                this.drawSvgCanvas.drawRectangle(pagingTextRect, this.paginggroup);
                var rightArrowRect = new helper_3.RectOption(heatMap.element.id + '_rightArrow', 'none', { color: 'transparent', width: 0 }, 1, new helper_3.Rect(rightArrowX - iconSize, rightArrowY - iconSize / 2, iconSize, iconSize));
                ...
                var leftArrowRect = new helper_3.RectOption(heatMap.element.id + '_leftArrow', 'none', { color: 'transparent', width: 0 }, 1, new helper_3.Rect(leftX, rightArrowY - iconSize / 2, iconSize, iconSize));
                this.drawSvgCanvas.drawRectangle(leftArrowRect, this.paginggroup);
                var leftOption = new helper_1.PathOption(heatMap.element.id + '_Legend_leftarrow', 'gray', 0.01, '#A0A0A0', 1, '0,0', leftArrow);
                var rightOption = new helper_1.PathOption(heatMap.element.id + '_Legend_rightarrow', 'gray', 0.01, '#A0A0A0', 1, '0,0', rightArrow);
        ...
        Legend.prototype.calculateGradientScale = function (scale) {
            ...
            this.legendRectScale = new helper_3.Rect(left, top, width, height);
            if (heatMap.paletteSettings.type === 'Gradient' || heatMap.paletteSettings.type === 'Fixed' &&
                heatMap.legendSettings.enableSmartLegend) {
        ...
        Legend.prototype.renderColorAxisGrid = function (legendRect) {
            ...
            for (var i = 0; i < (heatMap.isColorRange ? this.segmentCollections.length : heatMap.legendColorCollection.length); i++) {
                ...
                var direction = new helper_1.Line(pathX1, pathY1, pathX2, pathY2);
                var line = new helper_1.LineOption(this.heatMap.element.id + '_ColorAxis_Grid' + i, direction, '#EEEEEE', 1);
        ...
        Legend.prototype.renderLegendTitleTooltip = function (e, pageX, pageY) {
            if (e.target.id.indexOf('_legendTitle') !== -1 && e.target.textContent.indexOf('...') > -1) {
                helper_3.showTooltip(this.heatMap.legendSettings.title.text, pageX, pageY, this.heatMap.element.offsetWidth, this.heatMap.element.id + '_legendTitle_Tooltip', helper_2.getElement(this.heatMap.element.id + '_Secondary_Element'), null, this.heatMap);
        ...
        Legend.prototype.renderLegendLabelTooltip = function (e, pageX, pageY) {
            if (e.target.id.indexOf('_Legend_Label') !== -1 && e.target.textContent.indexOf('...') > -1) {
                ...
                if (targetId.length === 2) {
                    ...
                    helper_3.showTooltip(this.labelCollections[index], pageX, pageY, this.heatMap.element.offsetWidth, this.heatMap.element.id + '_LegendLabel_Tooltip', helper_2.getElement(this.heatMap.element.id + '_Secondary_Element'), null, this.heatMap);
        ...
        Legend.prototype.renderListLegendMode = function (rect, translate) {
            ...
            this.legendLabelTooltip = [];
            var listRect;
            var size = new helper_2.Size(0, 0);
            var labelX = 0;
            var labelY = 0;
            var legendX = rect.x;
            var legendY = rect.y;
            ...
            for (var i = x; i < y; i++) {
                if (heatMap.legendSettings.showLabel) {
                    var text = this.labelCollections[i];
                    size = helper_2.measureText(text, heatMap.legendSettings.textStyle);
                }
                var legendEventArgs = {
                ...
                if (translate && heatMap.rendering && this.legendRange.length <= heatMap.colorCollection.length) {
                    var rectPosition = new helper_2.LegendRange(legendX, legendY, legendSize, legendSize, heatMap.colorCollection[i].value, true, this.currentPage);
                    rectPosition.visible = !ej2_base_1.isNullOrUndefined(this.visibilityCollections[i]) ? this.visibilityCollections[i] : true;
                    if (!legendEventArgs.cancel) {
                        this.legendRange.push(rectPosition);
                    }
                    else {
                        var rectPosition_1 = new helper_2.LegendRange(legendX, legendY, 0, 0, heatMap.colorCollection[i].value, true, this.currentPage);
                        this.legendRange.push(rectPosition_1);
                    }
                    if (heatMap.legendSettings.showLabel) {
                        var textPosition = new helper_2.LegendRange(labelX, (labelY - size.height / 2), size.width, size.height, heatMap.colorCollection[i].value, true, this.currentPage);
                        textPosition.visible = !ej2_base_1.isNullOrUndefined(this.visibilityCollections[i]) ? this.visibilityCollections[i] : true;
                        this.legendTextRange.push(textPosition);
                    }
                }
                if (!legendEventArgs.cancel) {
                    if (heatMap.legendSettings.showLabel) {
                        var text = helper_2.getTitle(this.labelCollections[i], heatMap.legendSettings.textStyle, textWrapWidth);
                        if (text[0].indexOf('...') !== -1 && heatMap.enableCanvasRendering) {
                            this.legendLabelTooltip.push(new helper_2.CanvasTooltip(this.labelCollections[i], new helper_3.Rect(labelX, labelY, size.width, size.height)));
                        }
                        var textBasic = new helper_1.TextBasic(labelX, labelY, 'start', text, 0, 'translate(0,0)', 'middle');
                        var options = new helper_1.TextOption(heatMap.element.id + '_Legend_Label' + i, textBasic, heatMap.legendSettings.textStyle, heatMap.legendSettings.textStyle.color || heatMap.themeStyle.legendLabel);
                    ...
                    listRect = new helper_3.Rect(legendX, legendY, legendSize, legendSize);
                    ...
                    var rectItems = new helper_3.RectOption(heatMap.element.id + '_legend_list' + i, listColor, tempBorder, 1, listRect);
        ...
        Legend.prototype.legendRangeSelection = function (index) {
            ...
            if (heatMap.enableCanvasRendering) {
                ...
                helper_3.removeElement(heatMap.heatMapSeries.containerRectObject.id);
                if (heatMap.cellSettings.showLabel) {
                    helper_3.removeElement(heatMap.heatMapSeries.containerTextObject.id);
                }
            }
            if (heatMap.legendSettings.enableSmartLegend) {
