    ...
    var AxisHelper = (function () {
        ...
        AxisHelper.prototype.calculateAxisSize = function (rect) {
            ...
            for (var i = 0, len = axisCollection.length; i < len; i++) {
                ...
                axis_1.rect = ej2_base_1.extend({}, rect, null, true);
        ...
        AxisHelper.prototype.drawXAxisLabels = function (axis, parent, rect) {
            ...
            for (var i = 0, len = labels.length; i < len; i++) {
                ...
                label = axis.enableTrim ? helper_1.textTrim(axis.maxLabelLength, labels[i], axis.textStyle) : label;
                ...
                if (angle % 360 !== 0) {
                    ...
                    var rotateSize = helper_1.rotateTextSize(axis.textStyle, label, angle);
                ...
                var options = new helper_1.TextOption(heatMap.element.id + '_XAxis_Label' + i, new helper_2.TextBasic(x, y, (angle % 360 === 0) ? 'start' : 'middle', label, angle, transform), axis.textStyle, axis.textStyle.color || heatMap.themeStyle.axisLabel);
                ...
                if (label.indexOf('...') !== -1) {
                    this.heatMap.tooltipCollection.push(new helper_1.CanvasTooltip(labels[i], new helper_1.Rect(x, y - elementSize.height, elementSize.width, elementSize.height)));
                }
                if (compactInterval !== 0) {
        ...
        AxisHelper.prototype.drawYAxisLabels = function (axis, parent, rect) {
            ...
            for (var i = 0, len = labels.length; i < len; i++) {
                ...
                label = axis.enableTrim ? helper_1.textTrim(axis.maxLabelLength, labels[i], axis.textStyle) : labels[i];
                var options = new helper_1.TextOption(heatMap.element.id + '_YAxis_Label' + i, new helper_2.TextBasic(x, y, anchor, label, 0, 'rotate(' + 0 + ',' + (x) + ',' + (y) + ')', 'middle'), axis.textStyle, axis.textStyle.color || heatMap.themeStyle.axisLabel);
                ...
                if (label.indexOf('...') !== -1) {
                    ...
                    this.heatMap.tooltipCollection.push(new helper_1.CanvasTooltip(labels[i], new helper_1.Rect(xValue, y - elementSize.height, elementSize.width, elementSize.height)));
                }
            }
            if (!heatMap.enableCanvasRendering) {
                parent.appendChild(labelElement);
