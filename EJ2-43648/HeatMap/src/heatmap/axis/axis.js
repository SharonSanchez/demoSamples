...
define(["require", "exports", "@syncfusion/ej2-base", "@syncfusion/ej2-data", "../model/base", "../model/theme", "../utils/helper", "../utils/helper"], function (require, exports, ej2_base_1, ej2_data_1, base_1, theme_1, helper_1, helper_2) {
    "use strict";
    ...
    var Axis = (function (_super) {
        ...
        function Axis() {
            ...
            _this.nearSizes = [];
            _this.farSizes = [];
            _this.maxLabelSize = new helper_1.Size(0, 0);
            _this.titleSize = new helper_1.Size(0, 0);
            _this.multilevel = [];
            _this.axisLabels = [];
            _this.tooltipLabels = [];
            _this.labelValue = [];
        ...
        Axis.prototype.computeSize = function (axis, heatmap, rect) {
            var size = new helper_1.Size(0, 0);
            var innerPadding = 10;
        ...
        Axis.prototype.multiPosition = function (axis, index) {
            ...
            var multiPosition = new helper_2.MultiLevelPosition(0, 0);
                ...
                multiLevel.categories.map(function (categoryLabel) {
                    ...
                    if (categoryLabel.text !== '' && categoryLabel.start !== null && categoryLabel.end !== null) {
                        ...
                        if ((labelSize.width > gap - padding) && (multiLevel.overflow === 'Wrap') && !isVertical) {
                            height = (height * (helper_2.textWrap(categoryLabel.text, gap - padding, multiLevel.textStyle).length));
                        }
        ...
        Axis.prototype.getTitleSize = function (axis, innerPadding) {
            var titleSize = new helper_1.Size(0, 0);
            if (this.title.text) {
                titleSize = helper_1.measureText(this.title.text, this.title.textStyle);
        ...
        Axis.prototype.getMaxLabelSize = function (axis, heatmap) {
            ...
            if (axis.orientation === 'Horizontal' && (axis.labelIntersectAction === 'Rotate45' ||
                ...
                for (var i = 0, len = labels.length; i < len; i++) {
                    ...
                    if (!axis.isInversed) {
                        if (ej2_base_1.isNullOrUndefined(previousEnd)) {
                        ...
                        if (ej2_base_1.isNullOrUndefined(previousStart)) {
                    ...
                    if (axis.orientation === 'Horizontal' && axis.labelIntersectAction === 'MultipleRows' && axis.labelRotation === 0) {
                        this.multipleRow.push(new base_1.MultipleRow(startPoint, endPoint, count, label, row));
                    }
                }
            ...
            for (var i = 0; i < labels.length; i++) {
                ...
                var label = void 0;
                if (axis.enableTrim) {
                    label = helper_1.textTrim(axis.maxLabelLength, labels[i], axis.textStyle);
                }
                else {
                    label = labels[i];
                }
                var size = (axis.angle % 180 === 0) ?
                    helper_1.measureText(label, axis.textStyle) : helper_1.rotateTextSize(axis.textStyle, labels[i], axis.angle);
        ...
        Axis.prototype.calculateNumericAxisLabels = function (heatmap) {
            ...
            min = !ej2_base_1.isNullOrUndefined(this.minimum) ? this.minimum : ((adaptorMin) ? adaptorMin : 0);
            max = !ej2_base_1.isNullOrUndefined(this.maximum) ? this.maximum :
            ...
            max = !ej2_base_1.isNullOrUndefined(this.maximum) ? max : (adaptorMax ? adaptorMax : (max + min));
            ...
            for (var i = min; i <= max; i = i + (interval * this.increment)) {
                var value = helper_1.formatValue(isCustom, format, i, this.format);
        ...
        Axis.prototype.calculateCategoryAxisLabels = function () {
            ...
            var min = !ej2_base_1.isNullOrUndefined(this.minimum) ? this.minimum : 0;
            var max = !ej2_base_1.isNullOrUndefined(this.maximum) ? this.maximum : this.maxLength;
            var interval = this.interval ? this.interval : 1;
            var temp;
            if (!ej2_base_1.isNullOrUndefined(this.minimum) && !ej2_base_1.isNullOrUndefined(this.maximum) && min > max) {
                temp = min;
                min = max;
                max = temp;
            }
        ...
        Axis.prototype.calculateDateTimeAxisLabel = function (heatmap) {
            ...
            if (minimum === null && maximum === null) {
                ...
                if (minimum !== null && maximum === null) {
                    min = Date.parse(dateParser(dateFormatter(new Date(ej2_data_1.DataUtil.parse.parseJson({ val: minimum }).val))));
                    max = helper_1.increaseDateTimeInterval(min, this.maxLength, this.intervalType, this.increment).getTime();
                }
                else if (minimum === null && maximum !== null) {
                    max = Date.parse(dateParser(dateFormatter(new Date(ej2_data_1.DataUtil.parse.parseJson({ val: maximum }).val))));
                    min = helper_1.increaseDateTimeInterval(max, -this.maxLength, this.intervalType, this.increment).getTime();
                }
                else {
                    min = Date.parse(dateParser(dateFormatter(new Date(ej2_data_1.DataUtil.parse.parseJson({ val: minimum }).val))));
                    max = Date.parse(dateParser(dateFormatter(new Date(ej2_data_1.DataUtil.parse.parseJson({ val: maximum }).val))));
                ...
                while (tempInterval <= max) {
                    ...
                    tempInterval = helper_1.increaseDateTimeInterval(tempInterval, interval, this.intervalType, this.increment).getTime();
                }
                this.min = 0;
                this.axisLabelInterval = interval;
                ...
                while (tempInterval <= max) {
                    ...
                    tempInterval = helper_1.increaseDateTimeInterval(tempInterval, 1, this.intervalType, this.increment).getTime();
                }
            }
            this.labelValue = this.isInversed ? this.labelValue.reverse() : this.labelValue;
        };
        ...
        __decorate([
            ej2_base_1.Complex({ text: '', textStyle: theme_1.Theme.axisTitleFont }, base_1.Title)
        ...
        __decorate([
            ej2_base_1.Property(35)
        ], Axis.prototype, "maxLabelLength", void 0);
        __decorate([
            ej2_base_1.Complex({ color: '#b5b5b5', width: 0, type: 'Rectangle' }, base_1.AxisLabelBorder)
        ], Axis.prototype, "border", void 0);
        __decorate([
            ej2_base_1.Collection([], base_1.MultiLevelLabels)
